...
When `DISABLE_HW_OVERLAYS=1`, Android forces SurfaceFlinger to use GPU/client composition instead of hardware overlay planes for final display composition.

This is a compatibility workaround, not a guaranteed performance loss or gain. Hardware overlays are usually more efficient when they work correctly, but the real impact depends on the app, layer stack, refresh rate, and driver behavior. It may slightly affect battery, heat, or GPU headroom in demanding cases, or it may be barely noticeable.
